    ...
    nuxt.hook("modules:done", async () => {
      ...
      if (resolvedAutoI18n && resolvedAutoI18n.locales && resolvedAutoI18n.strategy !== "no_prefix") {
        ...
        for (const group of config.groups.filter((g) => !g._skipI18n)) {
          group.allow = util.asArray(group.allow || []).map((path) => splitPathForI18nLocales(path, i18n)).flat();
          group.disallow = util.asArray(group.disallow || []).map((path) => splitPathForI18nLocales(path, i18n)).flat();
        }
      }
